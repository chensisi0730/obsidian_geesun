...
打通"遥操作→数据采集→仿真训练→真机部署"端到端 pipeline，用桌面场景验证 G1 灵巧操作能力

### 关键任务

- G1 硬件联调（开机自检、关节标定）与动捕服/VR 遥操作搭建，实现动捕→G1 实时跟随.
- G1 硬件联调VR 遥操作搭建，实现VR →G1 实时跟随.
- Isaac Lab 仿真环境搭建（G1 URDF + 桌面方块物体 ）
- 部署 unitree/ lerobot 数据采集管线，最好能兼容使用第一视角视频
- 桌面场景（方块物品抓取/摆放）采集 ≥200 条演示数据，训练 ACT 系列算法的模仿学习
- 桌面场景采集 ≥200 条演示数据，训练 PI系列算法的模仿学习
- 首次 Sim2Real 部署：G1 真机运行训练策略，基于失败回放迭代数据集与策略
- openArm能力探知：数据采集，模型训练，模型部署，探索不同算法的模仿学习实现抓取苹果等物品
- 文档记录常见问题手册，MVP 演示。
- 风险：5090显存不够，无法支持lora训练, 即使后续使用h200,前期验证阶段还是要在本地工作站验证，本地需要一块pro6000D 。多人使用同一台机器人G1，可能会有干扰。
...
构建 CATL 电池产线或者肇庆工厂数字孪生场景，选定 1 个具体工序和任务，在仿真中可交互操作并完成工序演示数据采集。

### 关键任务

- 尝试宇树 RL Lab 用 RL 微调 G1 运动控制，预研行走+操作联合策略
- 产线现场调研与需求确认：选定工序（初步选定优先电池外观缺陷检测或不合格电芯取放），输出工序定义sop文档
- 产线 3D 资产收集与建模（工位、传送带、电池、夹具），Isaac Lab 场景搭建（刚体物理属性、碰撞网格优化）
- 引入结构工程师支持数字仿真环境创建：工位/夹具/治具 CAD 建模、尺寸，关节限位，惯量，质量，质心校核、数字仿真环境结构还原、现场结构适配支持，提升仿真场景可落地性。
- 部署Genie Sim 仿真平台，利用llm和rag并行搭建和生成产线场景，作为对标与快速迭代环境，机器人仿真基础设施的搭建和应用到快速产生符合需要的工业或者家庭训练场景。
- G1 双臂交互验证（可达空间、运动规划）
- 实体机器人采集产线工序演示数据（包括仿真遥操作或程序化增强生成）200+ 轨迹，接入数据增强管线，
- 基于各种来源的数据，训练不同的VLA算法，包括GR00T系列，给出评估报告，并且部署
- 基于各种来源的数据，训练不同的VLA算法，包括pi系列，给出评估报告，并且部署
- 基于各种来源的数据，训练不同的VLA算法，包括act系列，给出评估报告，并且部署

### 关键决策

- **Isaac Lab vs Genesis**：以 Isaac Lab 为主（高保真），Genesis 为辅助验证（快速迭代）。若 Genesis 成熟度足够，Phase 4 可迁移至 Genesis 做环境预测模型训练环境。
- **工序选择原则**：选一个动作相对简单、评估指标清晰的工序（如电池外观检测：走到工位→拿起电池→旋转检查→放回），避免一开始就选需要全身协调的复杂工序。

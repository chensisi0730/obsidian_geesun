...
- 桌面场景（方块物品抓取/摆放）采集 ≥200 条演示数据，训练 ACT 策略，探索不同算法的模仿学习
...
- Genesis 机器人仿真基础设施的搭建和应用到快速产生符合需要的工业或者家庭训练场景。
- 产线现场调研与需求确认：选定工序（初步选定优先电池外观缺陷检测或不合格电芯取放），输出工序定义sop文档
- 产线 3D 资产收集与建模（工位、传送带、电池、夹具），Isaac Lab 场景搭建（刚体物理属性、碰撞网格优化）
- 引入结构工程师支持数字仿真环境创建：补齐工件 CAD、夹具/治具结构、关键尺寸与装配关系，提升仿真场景可落地性。
- 部署Genie Sim 仿真平台，并行搭建和生成产线场景，作为对标与快速迭代环境
- G1 双臂交互验证（可达空间、运动规划）
- 实体机器人采集产线工序演示数据（包括仿真遥操作或程序化增强生成）200+ 轨迹，接入数据增强管线，训练VLA算法并且部署。
...
| **结构工程师（外部支持）**    | 工位/夹具/治具 CAD 建模、尺寸，关节限位，惯量，质量，质心校核、数字仿真环境结构还原、现场结构适配支持             |
| 数据采集人员1人（外包）       |                                                |
